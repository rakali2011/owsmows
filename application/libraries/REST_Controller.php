...
//            $valid_response = $this->register_model->is_valid_api_key($auth_key);
            $valid_response = $this->jwt->decode($auth_key, SECRET_KEY);
            if ($valid_response) {


                $issued_time = $valid_response->issuedAt;
                $today = date("Y-d-m H:i:s");
//                $today = new DateTime();
                $today = strtotime($today);
                $end_date = strtotime($issued_time);
                
//                $end_date = DateTime::createFromFormat('Y-d-m H:i:s', $issued_time);
                
//                $diff = $end_date->diff($today);
//                
//                $diff_minutes =  ($diff->days * 86400 + $diff->h * 3600 + $diff->i * 60 + $diff->s)/60;
//                $diff_minutes = floor($diff_minutes);
                $diff_minutes = round(abs($end_date - $today)/60,2);
                ...
                $this->user_id = $valid_response->userId;
            } else {
                $response["error"] = "true";
